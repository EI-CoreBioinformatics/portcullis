...
true), memory_saving_splitting(false), splitrule(DEFAULT_SPLITRULE), predict_all(false), keep_inbag(false), sample_fraction(1), num_threads(
...
void Forest::init(MemoryMode memory_mode, uint mtry, std::string output_prefix,
...
void Forest::init(std::string dependent_variable_name, MemoryMode memory_mode, Data* input_data, uint mtry,
    ...
    this->init(memory_mode, mtry, output_prefix, num_trees, seed, num_threads,
            importance_mode, min_node_size, prediction_mode, sample_with_replacement,
            memory_saving_splitting, splitrule, predict_all, sample_fraction);
    
    this->setData(input_data, dependent_variable_name, status_variable_name, unordered_variable_names);    
}
...
* @param data
*/
void Forest::setData(Data* data, std::string dependent_variable_name, std::string status_variable_name, std::vector<std::string>& unordered_variable_names) {

   // Record the pointer to the data object
   this->data = data;
...
void Forest::writeOutput(std::ostream* out) {

    *out << std::endl;
    writeOutputInternal();
    *out << "Dependent variable name:           " << data->getVariableNames()[dependent_varID] << std::endl;
    *out << "Dependent variable ID:             " << dependent_varID << std::endl;
    *out << "Number of trees:                   " << num_trees << std::endl;
    *out << "Sample size:                       " << num_samples << std::endl;
    *out << "Number of independent variables:   " << num_independent_variables << std::endl;
    *out << "Mtry:                              " << mtry << std::endl;
    *out << "Target node size:                  " << min_node_size << std::endl;
    *out << "Variable importance mode:          " << importance_mode << std::endl;
    *out << "Memory mode:                       " << memory_mode << std::endl;
    *out << "Seed:                              " << seed << std::endl;
    *out << "Number of threads:                 " << num_threads << std::endl;
    *out << std::endl;

    if (prediction_mode) {
        writePredictionFile();
    } else {
        *out << "Overall OOB prediction error:      " << overall_prediction_error << std::endl;
        *out << std::endl;
