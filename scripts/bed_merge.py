...
def main():
	parser = argparse.ArgumentParser(
		"Script to merge multiple BED files, using target sequence, and thick_start and thick_end values to generate set keys.  (Only entering a single file here essentially copies the file).  Merged BED entries contain the maximum score found across all input files.")
	...
	parser.add_argument("--operator", default="total",
						help="Operator to use for calculating the score in the merged BAM file.  Options: [max, total]")
	...
	parser.add_argument("-s", "--ignore_strand", action='store_true', default=False,
						help="Ignore strand information for calculating match in bed files")
	parser.add_argument("-p", "--prefix", default="Junc_intersected",
