...
rule bam_sort:
	input: ALIGN_DIR+"/output/{aln_method}-{reads}.bam"
	output: ALIGN_DIR+"/output/{aln_method}-{reads}.sorted.bam"
	threads: int(THREADS)
	message: "Using samtools to sort {input}"
	shell: "{LOAD_SAMTOOLS}; samtools sort -o {output} -O bam -m 1G -T sort_{wildcards.aln_method}_{wildcards.reads} -@ {threads} {input}"
