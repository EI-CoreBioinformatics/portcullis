...
class PEntry:
    ...
    def __str__(self):
        return self.junc + "\t" + self.aligner + "\t" + str(self.tp) + "\t" + str(self.tn) + "\t" + str(self.fp) + "\t" + str(self.fn) \
    ...
    def calc_scores(self):
        ...
        self.acc = (float(self.tp + self.tn) / float(self.tp + self.fp + self.tn + self.fn)) * 100.0 if self.sum > 0 else 0.0
        self.f1 = (float(2 * self.tp) / float(2 * self.tp + self.fp + self.fn)) * 100.0 if self.tp + self.fp + self.fn > 0 else 0.0

    @staticmethod
    def header():
        return "JuncTool\tAligner\tTP\tTN\tFP\tFN\tSEN\tSPC\tPRC\tACC\tF1"
