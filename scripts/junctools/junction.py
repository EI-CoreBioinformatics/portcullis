...
class JuncFactory(Enum):
    ...
    def one_line_parse(self):
        return False if self.GFF or self.EGFF or self.GTF else True
...
class Junction(object):
    ...
    @staticmethod
    def createJuncSet(filepath, keyonly=False):
    ...
    @staticmethod
    def createSpliceSiteSet(filepath, use_strand=True):

        items = set()

        with open(filepath) as f:
            for line in f:
                junc = Junction.create(filepath, use_strand=use_strand)
                res = junc.parse_line(line, fullparse=False)
                if res == None:
                    continue
...
class ExonJunction(Junction):
    ...
    def __init__(self, use_strand=True, junc_to_copy=None):
        ...
        if junc_to_copy and junc_to_copy is ExonJunction:
...
class BedJunction(ExonJunction):
    def __init__(self, use_strand=True, junc_to_copy=None):
        ...
        if junc_to_copy and junc_to_copy is BedJunction:
            self.red = 0
            self.green = 0
            self.blue = 0
            self.block_count = 2
            self.block_sizes = [0, 0]
    ...
    def __str__(self, style=JuncFactory.IBED):

        line = ""

        if style == JuncFactory.BED6:

            line = [self.refseq, self.start, self.end + 1, self.name, self.score,
            ...
            if style == JuncFactory.IBED:
                ...
                if style == JuncFactory.EBED:

                    line = [self.refseq, self.left, self.right + 1, self.name, self.score,
                            self.strand if self.strand else ".",
                            self.start, self.end + 1,
                            rgb,
                            self.block_count,
...
class GFFExonJunction(ExonJunction):
...
class TabJunction(ExonJunction):
    def __init__(self, use_strand=True, junc_to_copy=None):
        ...
        if junc_to_copy and junc_to_copy is TabJunction:
    ...
    def parse_line(self, line, fullparse=True):
        ...
        if len(parts) != 56 and len(parts) > 1:
            msg = "Unexpected number of columns in TAB file.  Expected 56, found " + str(len(parts))
