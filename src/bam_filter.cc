    ...
    for(size_t i = 0; i < al.getNbCigarOps(); i++) {
        ...
        if (op.type == BAM_CIGAR_REFSKIP_CHAR) {
            ...
            shared_ptr<Intron> location = make_shared<Intron>(RefSeq(refId, refName, refLength), lEnd, rStart - 1, 
                strandSpecific != StrandSpecific::UNSTRANDED ? portcullis::strandFromBool(al.isReverseStrand()) : UNKNOWN);                        
...
BamAlignmentPtr portcullis::BamFilter::clipMSR(const BamAlignment& al, vector<RefSeq>& refs, JunctionSystem& js, bool& allBad) {
    ...
    for(size_t i = 0; i < al.getNbCigarOps(); i++) {
        ...
        if (op.type == BAM_CIGAR_REFSKIP_CHAR) {
            ...
            // Create the intron
            shared_ptr<Intron> location = make_shared<Intron>(RefSeq(refId, refName, refLength), lEnd, rStart - 1, 
                strandSpecific != StrandSpecific::UNSTRANDED ? portcullis::strandFromBool(modifiedAlignment->isReverseStrand()) : UNKNOWN);     
