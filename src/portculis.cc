...
int main(int argc, char *argv[])
{
    ...
    try {
        ...
        hidden_options.add_options()
            ("sorted-bam-file,i", po::value<string>(&bam_file), "Path to the sorted BAM file to process.")
            ("genome-file,i", po::value<string>(&genome_file), "Path to the genome file to process.")
        ;

        // Positional option for the input bam file
        po::positional_options_description p;
        p.add("sorted-bam-file", 1);
        p.add("genome-file", 2);
        ...
        if (vm.count("sorted-bam-file"))
        {
            bam_file = vm["sorted-bam-file"].as<string>();
        }
